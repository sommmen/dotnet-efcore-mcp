...
| `preview_query_sql` | `contextName: string`, `query: string`, `connectionName?: string`, `targetName?: string` | The provider-generated SQL for a `run_query`-style expression, obtained from the compiled, unexecuted `IQueryable` via `ToQueryString()`. Requires `QueryExecution:Mode` to be `InProcess`. Never opens a database connection, runs a command, or reads/writes rows. Rejects scalar/element results, already-materialized results, and non-translatable operators (e.g. `Zip`) with a message directing the caller to `run_query` instead |
