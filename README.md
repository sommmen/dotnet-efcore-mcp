...
| `preview_query_sql` | `contextName: string`, `query: string`, `connectionName?: string`, `targetName?: string` | The provider-generated SQL for a `run_query`-style expression, obtained from the compiled, unexecuted `IQueryable` via `ToQueryString()`. Requires `QueryExecution:Mode` to be `InProcess`. The `ToQueryString()` call itself never opens a database connection, runs a command, or reads/writes rows (though a caller-supplied expression forcing early enumeration before that point is not prevented). Rejects scalar/element results, already-materialized results, and non-translatable operators (e.g. `Zip`) with a message directing the caller to `run_query` instead |
