...
creates or executes a database command, or calls `SaveChanges`. Statement-mode queries that
produce a scalar, list, or other client-side `IEnumerable` are rejected for preview. See
