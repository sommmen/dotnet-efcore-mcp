...
whose final result is still an `IQueryable`; it never enumerates the query, opens a connection,
creates or executes a database command, or calls `SaveChanges`. Requires `QueryExecution:Mode` to be
configured as `InProcess`; the tool rejects all non-`InProcess` modes. Queries are also rejected if
