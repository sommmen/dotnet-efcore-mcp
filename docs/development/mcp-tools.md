...
creates or executes a database command, or calls `SaveChanges`. Queries are rejected if they use
statement-mode syntax (multiple statements or trailing `;` — the same single-expression restriction
`run_query` enforces), or if their final value is a scalar, list, or other client-side `IEnumerable`
rather than `IQueryable` (since there is no SQL to preview for those). See
