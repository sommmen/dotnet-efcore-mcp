...
whose final result is still an `IQueryable`; the `ToQueryString()` call itself never enumerates the
query, opens a connection, creates or executes a database command, or calls `SaveChanges`, though a
caller-supplied expression that forces enumeration before that point is not prevented. Requires
`QueryExecution:Mode` to be configured as `InProcess`; the tool rejects all non-`InProcess` modes. Queries are also rejected if
