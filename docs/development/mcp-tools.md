...
- [x] `preview_query_sql` — return the provider-generated SQL for a `run_query`-style query without executing it
...
Successful **sequence-returning** `run_query` responses carry `hasMoreRows: boolean`, reporting
whether another row matches after the effective `skip` and effective `take` window without adding a
row to `rows`, changing `rowCount`, or exposing a total count; `take: 0` always returns `rows: []`
and `hasMoreRows: false` without materializing or querying the database. Terminal scalar aggregates
have no page window and return no `hasMoreRows` signal. See
[`QueryExecutor.MaterializeWithContinuationAsync`](./query-execution.md#p0-2--run_query-continuation-indicator)
for the sentinel-row implementation and the executor/tool test coverage.
...
## P0 #4 — `preview_query_sql`

`preview_query_sql` is a read-only tool with the same request inputs as `run_query` (`contextName`,
`query`, optional `connectionName`, optional `targetName`). It returns a single `sql` string
containing the SQL generated by the configured EF Core provider's `ToQueryString()` for queries
whose final result is still an `IQueryable`. When successful, the `ToQueryString()` call itself 
never enumerates the query, opens a connection, creates or executes a database command, or calls 
`SaveChanges` — however, a caller-supplied expression may force enumeration or side effects before 
that point. Requires `QueryExecution:Mode` to be configured as `InProcess`; the tool rejects all 
non-`InProcess` modes. Queries are also rejected if
they use statement-mode syntax (the same single-expression/multi-statement restriction `run_query`
enforces, with `NormalizeAndGetRoot` stripping a single trailing `;` and rejecting multiple/compound
statements), or if their final value is a scalar, list, or other client-side `IEnumerable` rather than
`IQueryable` (since there is no SQL to preview for those). See
[Query execution](./query-execution.md#preview_query_sql) for the shared root-resolution/
authorization/Roslyn-compilation path with `run_query`, the sanitization guarantees, and the
tool/executor test coverage.
...
defaults to 10 and cannot exceed 25, with invalid counts rejected, and returns `truncated` whenever
...
## P2 #15 — named multi-target assembly registry

`load_assembly` accepts an optional `targetName` parameter so a client can register (or
reload) a compiled assembly under a stable logical name instead of always replacing the
server's single implicit target. `list_loaded_assemblies` returns every registered target's
name, source path, load timestamp, and current-default flag; `select_target` changes only
which registered name resolves when a call omits `targetName`, without unloading any other
target. The same optional `targetName` parameter, resolved the same way as `connectionName`,
is accepted by every tool that resolves an assembly-derived `DbContext` type: `list_contexts`,
`get_schema`, `run_query`, and `run_sql_query`.

Omitting `targetName` anywhere behaves exactly as before this feature: a single implicit
target is loaded/replaced and used, with no new required parameters and no response-shape
changes for existing callers. See [Multi-target assembly registry](./assembly-registry.md) for
the full configuration surface, isolation/caching/lifecycle model (each named target keeps its
own collectible `AssemblyLoadContext` and reload watcher; `SchemaCache`'s `Type`-keyed cache
already isolates correctly across targets), and how this composes with the per-connection
`AccessPolicy` (unaffected — policy stays scoped to the selected connection, not the selected
target).
